...
class TestModel(CompressionModel):
    ...
    def __init__(self, N=128, M=192, image_size=(384, 384), depth=[2, 0, 2, 0], heads=4, dim_head=192, dropout=0.1, **kwargs):
        ...
        self.lmbda = [0.001, 0.005, 0.01, 0.025, 0.05, 0.1]
        self.levels = len(self.lmbda)  # 8
        self.Gain = torch.nn.Parameter(torch.tensor(
            [ 1.0000,  2.2361,  3.1623,  5.0000,  7.0711, 10.0000]), requires_grad=True)
        self.InverseGain = torch.nn.Parameter(torch.tensor(
            [1.0000, 0.4472, 0.3162, 0.2000, 0.1414, 0.1000]), requires_grad=True)

    @property
    def downsampling_factor(self) -> int:
        return 2 ** (4 + 2)

    def forward(self, x, noisequant=False, training_stage=3, s=1):
        if training_stage > 1:
            if s != 0:
                factor = torch.max(self.Gain[s], torch.tensor(1e-4)) + eps
            else:
                s = 0
                factor = self.Gain[s].detach()
        else:
            s = 5
            factor = self.Gain[s].detach()

        refactor = 1.0 / factor.clone().detach()
        if noisequant:
            y = self.g_a(x)
            z = self.h_a(y)
            ...
            y_hat = self.gaussian_conditional.quantize(y * factor,
                                                       "noise" if self.training else "dequantize") * refactor
            ctx_params = self.context_prediction(y_hat)
            gaussian_params = self.entropy_parameters(
                torch.cat((params, ctx_params), dim=1)
            )
            scales_hat, means_hat = gaussian_params.chunk(2, 1)
            _, y_likelihoods = self.gaussian_conditional(y * factor, scales_hat * factor, means=means_hat * factor)
    ...
    def compress(self, x, s, step=0, factormode=False):
        ...
        if step != 0 and factormode:
            factor = step

        else:
            factor = torch.abs(self.Gain[s])
        refactor = torch.tensor(1.0) / factor
    ...
    def _compress_ar(self, y_hat, params, height, width, kernel_size, padding, factor, refactor):
        ...
        for h in range(height):
            for w in range(width):
                ...
                indexes = self.gaussian_conditional.build_indexes(scales_hat*factor)

                y_crop = y_crop[:, :, padding, padding]
                y_q = self.gaussian_conditional.quantize((y_crop-means_hat)*factor, "symbols", )
                y_hat[:, :, h + padding, w + padding] = y_q * refactor + means_hat
    ...
    def decompress(self, strings, shape, s, step=0, factormode=False):
    ...
    def _decompress_ar(
            self, y_string, y_hat, params, height, width, kernel_size, padding, factor, refactor,
    ):
        cdf = self.gaussian_conditional.quantized_cdf.tolist()
        cdf_lengths = self.gaussian_conditional.cdf_length.tolist()
        offsets = self.gaussian_conditional.offset.tolist()
        ...
        for h in range(height):
            for w in range(width):
                ...
                rv = self.gaussian_conditional.dequantize(rv*refactor, means_hat)
...
if __name__ == "__main__":
    ...
    model = TestModel(N=128, M=192,  image_size=(384, 384), depth=[2, 0, 2, 0], heads=4, dim_head=192, dropout=0.1)
    ...
    # print(model)
    out = model(input)
    flops, params = get_model_complexity_info(model, (3, 384, 384), as_strings=True, print_per_layer_stat=True,flops_units=" ", param_units= " ")
